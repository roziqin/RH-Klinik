...
if($_GET['ket']=='tambahmenu'){
	...
	$sql = "INSERT INTO transaksi_gudang (transaksi_gudang_nota_print,transaksi_gudang_tanggal,transaksi_gudang_bulan,transaksi_gudang_waktu,transaksi_gudang_member,transaksi_gudang_total,transaksi_gudang_diskon,transaksi_gudang_tax,transaksi_gudang_tax_service,transaksi_gudang_bayar,transaksi_gudang_type_bayar,transaksi_gudang_user,transaksi_gudang_therapist,transaksi_gudang_nama,transaksi_gudang_ket,transaksi_gudang_bayar_debet,transaksi_gudang_bayar_debet_ket,transaksi_gudang_cs,transaksi_gudang_kasir) VALUES ('$nota_print','$tgl','$bln','$wkt','0','0','0','0','0','0','$paytype','$user','0','','','0','','0','0')" ;

	mysqli_query($con,$sql);

	$qn= "SELECT MAX( transaksi_gudang_id ) AS nota FROM transaksi_gudang where transaksi_gudang_user='$user'";
	...
	while($baris = mysqli_fetch_assoc($result)) {
        ...
        if($datastok['barang_set_stok']!=0) {
	        ...
	        $sql1 = "INSERT into log_stok(user,barang,stok_awal,stok_jumlah,tanggal,alasan,keterangan,tempat)values('$user','$barang','$awalgudang','$jml_stok_gudang','$tgl','','transaksi','Gudang')";
			mysqli_query($con,$sql1);
	...
	if ($datacek['jml']!=0) {
		...
		$sql1="SELECT count(transaksi_gudang_id) as jumlah, sum(transaksi_gudang_total) as total, sum(transaksi_gudang_diskon) as diskon from transaksi_gudang where transaksi_gudang_tanggal='$tgl' and transaksi_gudang_user = '$user' group by transaksi_gudang_tanggal";
		$query1=mysqli_query($con,$sql1);
		$data1=mysqli_fetch_assoc($query1);

		$sql2="SELECT count(transaksi_gudang_id) as jumlah, sum(transaksi_gudang_total) as debet, sum(transaksi_gudang_diskon) as diskon from transaksi_gudang where transaksi_gudang_tanggal='$tgl' and transaksi_gudang_user = '$user' and transaksi_gudang_type_bayar='Debet' group by transaksi_gudang_tanggal";
		$query2=mysqli_query($con,$sql2);
		$data2=mysqli_fetch_assoc($query2);

		$sql3="SELECT count(transaksi_gudang_id) as jumlah, sum(transaksi_gudang_total) as cash, sum(transaksi_gudang_diskon) as diskon from transaksi_gudang where transaksi_gudang_tanggal='$tgl' and transaksi_gudang_user = '$user' and transaksi_gudang_type_bayar='Cash' group by transaksi_gudang_tanggal";
