    ...
    <div class="modal-content">
      ...
      <form method="post" class="form-stok">
        ...
        <div class="modal-footer d-flex justify-content-center">
          <button class="btn btn-primary" id="submit-stok-gudang" data-dismiss="modal" aria-label="Close">Proses</button>
          <button class="btn btn-primary" id="update-stok-gudang" data-dismiss="modal" aria-label="Close" disabled="true">Proses</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      $( "#defaultForm-ket" ).change(function() {
        if($(this).val()!='') {
          $('#update-stok-gudang').removeAttr("disabled");
        } else {
          $('#update-stok-gudang').attr("disabled","true");
        }
      });
      $("#submit-stok-gudang").click(function(){
        var data = $('#modalstok .form-stok').serialize();
        $.ajax({
          type: 'POST',
          url: "controllers/stok.ctrl.php?ket=submit-stok-gudang",
          data: data,
          success: function() {
            console.log("sukses")
      ...
      $("#update-stok-gudang").click(function(){
        var data = $('#modalstok .form-stok').serialize();
        $.ajax({
          type: 'POST',
          url: "controllers/stok.ctrl.php?ket=update-stok-gudang",
          data: data,
          success: function() {
